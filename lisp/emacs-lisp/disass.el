...
(defvar disassemble-column-1-indent 19 "*")
(defvar disassemble-column-2-indent 21 "*")
...
OBJ should be a call to BYTE-CODE generated by the byte compiler."
  (let (bytes constvec arg-spec)
	...
	(setq lap (cdr lap))
	(indent-to indent)
        (when (and stack-depths pc-value)
          (insert (format "[stack %d] " (aref stack-depths pc-value))))
	(if (eq 'TAG op)
	    (progn
